...
const camaraArribaIzq = new THREE.PerspectiveCamera(50, 1, 0.1, 600);
var frustumSize = 20;
var aspect = document.getElementById("top-camera").clientWidth /
...
const canvas3 = (document.getElementById("top-camera"));
// const canvas4: HTMLCanvasElement = <HTMLCanvasElement>document.getElementById("c4")
const rendererArribaIzq = new THREE.WebGLRenderer({
    canvas: canvas1,
});
rendererArribaIzq.setSize(document.getElementById("c1").clientWidth, document.getElementById("c1").clientHeight);
const rendererAbajo = new THREE.WebGLRenderer({
    canvas: canvas3,
});
rendererAbajo.setSize(document.getElementById("top-camera").clientWidth, document.getElementById("top-camera").clientHeight);
...
const sceneSection = new THREE.Scene();
const rendererArribaDer = new THREE.WebGLRenderer({
    canvas: canvas2,
});
rendererArribaDer.setSize(document.getElementById("c2").clientWidth, document.getElementById("c2").clientHeight);
...
const camaraArribaDer = new THREE.OrthographicCamera((frustumSize * aspect) / -2, (frustumSize * aspect) / 2, frustumSize / 2, frustumSize / -2, 0.1, 100);
...
const controlArribaDer = new OrbitControls(camaraArribaDer, rendererArribaDer.domElement);
const controlArribaIzq = new OrbitControls(camaraArribaIzq, rendererArribaIzq.domElement);
const cubeSection = new THREE.Mesh(geometry, mat);
...
const camaraAbajo = new THREE.OrthographicCamera(-15, 15, 5, -5, 0.1, 2);
...
function getIntersections() {
    points.forEach((p) => {
        ...
        if (intersects.length > 0) {
            ...
            scene.add(line);
            let pointI = new THREE.Mesh(new THREE.SphereGeometry(0.125, 4, 2), new THREE.MeshBasicMaterial({
                color: "white",
            }));
